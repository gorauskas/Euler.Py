...
class Euler(BaseEuler):
    def solve(self):
        return sum([self._is_lychrel(x, 0) for x in range(10000)])

    def _is_lychrel(self, n, d):
        if d == 50:
            return 1
        ...
        if str(n) == str(n)[::-1]:
            ...
            return self._is_lychrel(n, d + 1)

    @property
    def answer(self):
        return ('There are %d Lychrel numbers below ten thousand.'
                % self.solve())
